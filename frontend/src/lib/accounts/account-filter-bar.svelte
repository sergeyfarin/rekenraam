...
<div class="mt-6 grid gap-3 lg:grid-cols-[minmax(12rem,1fr)_repeat(4,minmax(9rem,12rem))]">
  <label class="block">
    <span class="text-xs font-semibold uppercase tracking-[0.16em] text-muted">{m.accounts_filter_search()}</span>
    <input
      bind:value={query}
      type="search"
      class="mt-2 h-11 w-full rounded-2xl border border-border bg-surface px-4 text-sm text-foreground shadow-sm outline-none transition placeholder:text-muted focus:border-accent"
      placeholder={m.accounts_filter_search_placeholder()}
    />
  </label>

  <label class="block">
    <span class="text-xs font-semibold uppercase tracking-[0.16em] text-muted">{m.accounts_filter_status()}</span>
    <select bind:value={statusFilter} class="mt-2 h-11 w-full rounded-2xl border border-border bg-surface px-3 text-sm text-foreground">
  ...
  <label class="block">
    <span class="text-xs font-semibold uppercase tracking-[0.16em] text-muted">{m.accounts_filter_class()}</span>
    <select bind:value={classFilter} class="mt-2 h-11 w-full rounded-2xl border border-border bg-surface px-3 text-sm text-foreground">
  ...
  <label class="block">
    <span class="text-xs font-semibold uppercase tracking-[0.16em] text-muted">{m.accounts_group_label()}</span>
    <select bind:value={groupMode} class="mt-2 h-11 w-full rounded-2xl border border-border bg-surface px-3 text-sm text-foreground">
  ...
  <label class="block">
    <span class="text-xs font-semibold uppercase tracking-[0.16em] text-muted">{m.accounts_sort_label()}</span>
    <select bind:value={sortMode} class="mt-2 h-11 w-full rounded-2xl border border-border bg-surface px-3 text-sm text-foreground">
