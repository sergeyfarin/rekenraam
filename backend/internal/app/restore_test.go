...
func TestRestorePreservesUncheckpointedWALContent(t *testing.T) {
	...
	result, err := db.RestoreSQLiteDatabase(ctx, backupPath, sourceURL)
	require.NoError(t, err)
	require.NotEmpty(t, result.PreservedDir)

	preserved, err := sql.Open("sqlite", "file:"+filepath.Join(result.PreservedDir, "source.sqlite")+"?mode=ro")
	require.NoError(t, err)
	defer preserved.Close()
	var replaced int64
	require.NoError(t, preserved.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM audit_events WHERE operation = 'about.to.be.replaced'`).Scan(&replaced))
	assert.Equal(t, int64(1), replaced,
		"the preserved copy must hold what was only in the replaced database's WAL")
}
