...
func goldenTrading212Payload(t *testing.T) []byte {
	...
	payload := trading212FetchPayload{
		...
		Movements: []trading212Movement{
			...
			{ID: "ref-2", Type: "DIVIDEND", Timestamp: "2024-01-05T12:00:00Z", Amount: "4.32", Currency: "EUR", Notes: "AAPL dividend"},
			{ID: "ref-3", Type: "FEE", Timestamp: "2024-01-06T09:00:00Z", Amount: "-1.00", Currency: "EUR", Notes: "Custody fee"},
			{ID: "ref-4", Type: "WITHDRAWAL", Timestamp: "2024-01-10T10:00:00Z", Amount: "-200.00", Currency: "EUR", Notes: "Bank transfer"},
...
func TestTrading212Adapter_ParseGoldenPayload(t *testing.T) {
	...
	dividend := result.Rows[1]
	assert.Equal(t, "2024-01-05", dividend.Date)
	assert.Equal(t, "4.32", dividend.Amount)
