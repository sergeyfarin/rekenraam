...
class ReportDefinition(Base):
    ...
    previous_report_definition_id: Mapped[int | None] = mapped_column(
...
class ReportRun(Base):
    ...
    pricing_mode: Mapped[str] = mapped_column(
        String(32), nullable=False, server_default="latest_corrected"
    )
    pricing_policy_id: Mapped[int | None] = mapped_column(
        ForeignKey("pricing_policies.id", ondelete="SET NULL")
    )
