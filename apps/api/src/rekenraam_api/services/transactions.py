...
class TransactionService:
    ...
    async def list_transactions(self, filters: TransactionListFilters | None = None) -> TransactionPage:
        ...
        transactions, next_cursor = await self._repository.list_transactions(filters, allowed_book_ids)
        splits = await self._repository.list_splits_for_transaction_ids([transaction.id for transaction in transactions])
        return TransactionPage(items=tuple(self._map_transactions(transactions, splits)), next_cursor=next_cursor)
    ...
    async def update_transaction(self, transaction_id: int, input: TransactionMutationInput) -> TransactionSummary | None:
        ...
        if current is not None:
            ...
            await self._ensure_unlocked({split.account_id for split in current_splits}, current.occurred_date)
            await self._ensure_unlocked({split.account_id for split in input.splits}, input.txn_date)
        audit = self._access_policy.audit_stamp() if self._access_policy is not None else None
    ...
    async def delete_transaction(self, transaction_id: int) -> bool:
        ...
        splits = await self._repository.list_splits_for_transaction_ids([transaction.id])
        await self._ensure_unlocked({split.account_id for split in splits}, transaction.occurred_date)
        audit = self._access_policy.audit_stamp() if self._access_policy is not None else None
    ...
    async def list_account_register(self, account_id: int, *, limit: int = 100, cursor: str | None = None) -> RegisterPage:
        rows, next_cursor = await self._repository.list_account_register_splits(account_id, limit=limit, cursor=cursor)
    ...
    async def _validate_mutation_input(self, input: TransactionMutationInput) -> None:
        ...
        accounts = await self._repository.list_accounts_by_ids({split.account_id for split in input.splits})
        ...
        refs_valid = await self._repository.metadata_refs_belong_to_book(
            book_id=input.book_id,
            payee_id=input.payee_id,
            category_ids={split.category_id for split in input.splits if split.category_id is not None},
            ...
            project_ids={split.project_id for split in input.splits if split.project_id is not None},
        )
        if not refs_valid:
    ...
    async def _ensure_unlocked(self, account_ids: set[int], occurred_date) -> None:
        ...
        if locked:
            details = ", ".join(f"{account_id} (balanced through {lock_date})" for account_id, lock_date in locked)
            raise ValueError(f"cannot post transaction dated {occurred_date} to locked accounts: {details}")
