...
R10 core slices 1–4 complete; forecast screen slice next; T-76 and T-75b
...
and its snapshot, exact projection, authenticated API and constant-as-of FX
slices have landed. Forecast screen slice 5 is next in
`docs/plans/projected-balances-plan.md`; no forecast screen is implemented yet.
...
2. **R10 Projected balances — current, planned 2026-08-31:**
   ...
   acceptance follows M4; R8 remains next afterward. Slices 1–4 are complete:
   ...
   component-level exact rounding without changing source-currency results. It
   has no screen. **Next: slice 5 only, the forecast screen.** Loan helpers remain optional later
   work.
