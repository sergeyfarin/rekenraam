...
| Report URL contract | ✅ | The URL is the whole report state, and a link that arrives missing or with a malformed `start_date`/`end_date`/`bucket` is repaired in place — the view, grouping, mode, and every repeated ID filter survive (`lib/reports/report-range.ts`, tested). Rebuilding the URL from defaults instead used to discard the whole selection. |
