...
| Payee resolution on entry | 🟦 | A typed payee name resolves to the record that carries it on every write path, manual or import (exact match on the normalized name, which is unique among active payees). In the editor, a name no payee carries stops the save and offers fuzzy near matches before a record is created; confirmation is skipped when the name was not edited. 🟦 because existing rows still carry unlinked `payee_name` values — history needs its own tool (backlog T-44). |
...
| Report CSV export | ✅ | Every report view downloads its table as CSV with exact unformatted decimals and the commodity in its own column; shares export as integer basis points. RFC 4180 quoting, UTF-8 BOM for Excel. Filename names the report and its range. |
