...
| Spending / income read model | ✅ | `GET /reports/spending` ranks category or payee totals over a range, exact per commodity — the ranking compares whole-and-fractional units exactly, so amounts under one unit rank by value instead of all tying at zero, with within-commodity shares as integer basis points and a per-row drill-down query. Transfers cannot enter the basis (they have no category posting). `grouping_policy: direct_postings` — parents do not absorb children. |
...
| Report print layout | ✅ | Printing drops navigation, filters, the view switch, and the export button, keeping headings, the stated scope, tables, and charts. The interactive filter form is replaced on paper by the same context block the CSV carries, so a printout of a narrowed report is not indistinguishable from one over everything. Rows avoid page breaks and use borders that survive without printed backgrounds. |
