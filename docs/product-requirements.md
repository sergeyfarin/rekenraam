...
- MFA is TOTP (RFC 6238) plus single-use recovery codes, shipped 2026-08-07 (S-06). The design was selected over WebAuthn because this is a single-owner app routinely reached at a LAN address or private hostname, where origin binding is awkward and the recovery story for a lost authenticator is worse. Public deployment with real financial data now requires that the owner account actually **be enrolled**, not merely that the feature exist; enrolment requires `REKENRAAM_SECRET_KEY`, since the shared secret is sealed at rest. Public VPS deployment must otherwise use HTTPS and local authentication.
...
- Online connections are adapters users bring their own keys for, never coverage the product promises. Prefer file imports and token-based provider APIs; any aggregator integration must be bring-your-own-key and must not break the self-hosted privacy model or require the project to hold an intermediary license.
- Native desktop and mobile apps are out of scope: the single binary run locally is the desktop story, and responsive web (with PWA polish when justified) is the mobile story.
- Distribution before hosting: list the app on self-host marketplaces (for example PikaPods, Cloudron, Elestio, Umbrel) once the daily-driver core is complete enough for strong first impressions. A first-party hosted service is a separate business decision — deferred until marketplace/community demand evidence exists and MFA plus operational readiness are in place.
- Open source (decided 2026-07-02): the app will be open source under AGPL-3.0, with sole copyright retained so later dual-licensing or a first-party hosted offering stays possible. Closed source is rejected: trust and auditability are the value proposition of self-hosted finance software, and the distribution channels (marketplaces, self-host directories) effectively require an OSS license. If substantial external contributions are ever accepted, add a DCO or lightweight CLA at that point to preserve the dual-licensing option.
- Making the repository public and announcing the project are two separate events. The repo goes public quietly once the pre-public security gates pass (git-history secrets scrub, LICENSE, SECURITY.md with a private disclosure channel, secret/dependency scanning); the public announcement and marketplace listings wait until the daily-driver core (reports UI, exports, CSV import) ships, because the launch into self-host communities is a one-shot first impression. The concrete gated sequence lives in `docs/roadmap.md`.
- Security must never depend on the code being secret. Open-sourcing adds process requirements — private vulnerability disclosure, dependency and secret scanning, signed releases — not architectural ones.
...
The active near-term sequence and its acceptance boundaries live in
`docs/roadmap.md`. This document keeps the durable phase boundaries and product
intent; completed implementation plans remain design references only.
