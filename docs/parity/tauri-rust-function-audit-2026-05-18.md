...
| `db_storage.rs` | Desktop SQLite file location, file pickers, backup settings, scheduler, `VACUUM INTO`, restore/move/copy storage, DB health/stats/vacuum. Web replacement is container/Postgres operational docs, health/admin endpoints, and backup/restore smoke. | Safe, desktop-only except no web `VACUUM` analogue needed. |
