...
| commodities metadata including currencies | `src-tauri/src/db_currencies.rs`, `src-tauri/src/db_commodities.rs` | planned `apps/api/src/rekenraam_api/services/commodities.py`, `api/v1/commodities.py` | planned `/api/v1/commodities*` | not started | Treat `commodities` as the canonical asset master. Currencies should be modeled as commodity rows with `kind='currency'` rather than reintroducing a separate `currencies` table in PostgreSQL. |
