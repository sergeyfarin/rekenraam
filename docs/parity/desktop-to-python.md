...
| commodities metadata including currencies | `src-tauri/src/db_currencies.rs`, `src-tauri/src/db_commodities.rs` | `apps/api/src/rekenraam_api/repositories/metadata.py`, `apps/api/src/rekenraam_api/services/metadata.py`, `apps/api/src/rekenraam_api/api/v1/metadata.py`, `apps/api/src/rekenraam_api/repositories/pricing.py`, `apps/api/src/rekenraam_api/services/pricing.py`, `apps/api/src/rekenraam_api/api/v1/pricing.py` | `/api/v1/commodities`, `/api/v1/commodities/{id}`, `/api/v1/currencies`, `/api/v1/currencies/{id}`, `/api/v1/currencies/{id}/default`, `/api/v1/currencies/{id}/activation`, `/api/v1/pricing/sources`, `/api/v1/pricing/policy`, `/api/v1/pricing/source-assignments`, `/api/v1/pricing/refresh-state` | in progress | Commodity list/edit plus currency list/create/update/default/activation are now Python-backed. Active state is stored in `commodities.metadata_text` JSON, the default currency is always active, and inactive currencies remain visible in settings but are excluded from FX entry and refresh flows. The FX settings tab now uses Python-backed pricing endpoints for sources, persisted policy, source assignments, and refresh-state reads in web mode. Scheduled FX and market-price execution, including manual refresh, remains backend-owned worker work. Treat `commodities` as the canonical asset master. Currencies should be modeled as commodity rows with `kind='currency'` rather than reintroducing a separate `currencies` table in PostgreSQL. |
...
| pricing and FX refresh | `src-tauri/src/fx_refresh.rs`, `src-tauri/src/fx_rates/*` | `apps/api/src/rekenraam_api/repositories/pricing.py`, `apps/api/src/rekenraam_api/services/pricing.py`, `apps/api/src/rekenraam_api/api/v1/pricing.py`, planned worker slice | `/api/v1/pricing/sources`, `/api/v1/pricing/policy`, `/api/v1/pricing/source-assignments`, `/api/v1/pricing/refresh-state`, planned execution endpoints later | in progress | Pricing configuration, source assignments, and refresh-state reads are now Python-backed. The remaining gap is scheduled execution and manual refresh in a backend-owned worker rather than browser-owned automation. |
| pricing history and valuation state | `src-tauri/src/fx_refresh.rs`, `src-tauri/src/db_currencies.rs`, `src-tauri/migrations/V1__init.sql` | `apps/api/src/rekenraam_api/repositories/pricing.py`, `apps/api/src/rekenraam_api/services/pricing.py`, `apps/api/src/rekenraam_api/api/v1/pricing.py`, planned worker + valuation slice | `/api/v1/pricing/sources`, `/api/v1/pricing/policy`, `/api/v1/pricing/source-assignments`, `/api/v1/pricing/refresh-state`, planned valuation endpoints later | in progress | The Postgres baseline now includes `price_sources`, `pricing_policies`, `pricing_source_assignments`, and `pricing_refresh_state` for persisted pricing configuration and status. `price_ingest_runs`, `price_observations`, `commodity_price_sources`, valuation snapshots, and base-currency history are still pending. |
| settings and metadata | `src-tauri/src/db_accounts.rs`, `src-tauri/src/db_commodities.rs`, `src-tauri/src/db_storage.rs` | `apps/api/src/rekenraam_api/repositories/metadata.py`, `apps/api/src/rekenraam_api/services/metadata.py`, `apps/api/src/rekenraam_api/api/v1/metadata.py`, `apps/api/src/rekenraam_api/repositories/pricing.py`, `apps/api/src/rekenraam_api/services/pricing.py`, `apps/api/src/rekenraam_api/api/v1/pricing.py`, planned admin/settings modules later | `/api/v1/commodities`, `/api/v1/currencies`, `/api/v1/countries`, `/api/v1/institutions`, `/api/v1/categories`, `/api/v1/payees`, `/api/v1/tags`, `/api/v1/people`, `/api/v1/projects`, `/api/v1/pricing/sources`, `/api/v1/pricing/policy`, `/api/v1/pricing/source-assignments`, `/api/v1/pricing/refresh-state` | in progress | Institutions, categories, payees, tags, people, projects, commodity metadata, currency list/create/update/default/activation, and FX settings persistence are on Python endpoints. Remaining settings-side migration work is concentrated in app-level storage/admin flows plus backend execution for scheduled pricing refresh. |
