...
| commodities metadata including currencies | `src-tauri/src/db_currencies.rs`, `src-tauri/src/db_commodities.rs` | `apps/api/src/rekenraam_api/repositories/metadata.py`, `apps/api/src/rekenraam_api/services/metadata.py`, `apps/api/src/rekenraam_api/api/v1/metadata.py`, planned dedicated commodities/fx modules later | `/api/v1/commodities`, `/api/v1/commodities/{id}`, `/api/v1/currencies`, `/api/v1/currencies/{id}`, `/api/v1/currencies/{id}/default`, `/api/v1/currencies/{id}/activation`, planned `/api/v1/fx*` | in progress | Commodity list/edit plus currency list/create/update/default/activation are now Python-backed. Active state is stored in `commodities.metadata_text` JSON, the default currency is always active, and inactive currencies remain visible in settings but are excluded from FX entry and refresh flows. In the web app, pricing automation is now explicitly backend-owned: `CommoditySettings.svelte` no longer tries to run or restart a browser-side scheduler, and the remaining gap is a Python pricing service/worker slice for scheduled FX and market-price refresh, source assignments, and refresh status. Treat `commodities` as the canonical asset master. Currencies should be modeled as commodity rows with `kind='currency'` rather than reintroducing a separate `currencies` table in PostgreSQL. |
...
| pricing history and valuation state | `src-tauri/src/fx_refresh.rs`, `src-tauri/src/db_currencies.rs`, `src-tauri/migrations/V1__init.sql` | planned worker + API slice | planned `/api/v1/fx*`, `/api/v1/valuations*` | not started | Legacy schema includes `price_ingest_runs`, `price_observations`, `price_sources`, `commodity_price_sources`, `pricing_policies`, `pricing_source_assignments`, `pricing_refresh_state`, `valuation_snapshots`, `valuation_snapshot_items`, and `book_base_currency_history`. |
| settings and metadata | `src-tauri/src/db_accounts.rs`, `src-tauri/src/db_commodities.rs`, `src-tauri/src/db_storage.rs` | `apps/api/src/rekenraam_api/repositories/metadata.py`, `apps/api/src/rekenraam_api/services/metadata.py`, `apps/api/src/rekenraam_api/api/v1/metadata.py`, planned admin/settings modules later | `/api/v1/commodities`, `/api/v1/currencies`, `/api/v1/countries`, `/api/v1/institutions`, `/api/v1/categories`, `/api/v1/payees`, `/api/v1/tags`, `/api/v1/people`, `/api/v1/projects` | in progress | Institutions, categories, payees, tags, people, projects, commodity metadata, and currency list/create/update/default are on Python endpoints. Activation semantics, app-level storage/admin flows, and FX settings remain. |
