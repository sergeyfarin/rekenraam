...
| auth identity, sessions, and book access | `src-tauri/src/db.rs`, `src-tauri/src/session.rs` | `apps/api/src/rekenraam_api/repositories/access.py`, `apps/api/src/rekenraam_api/services/auth.py`, `apps/api/src/rekenraam_api/services/access.py`, `apps/api/src/rekenraam_api/services/request_context.py`, `apps/api/src/rekenraam_api/api/v1/auth.py` | `/api/v1/auth/*`, protected `/api/v1/*` routers | baseline complete | First-admin bootstrap, login/logout, user devices, auth sessions, request context, book membership enforcement, password change for the authenticated user, admin password reset, MFA (TOTP + recovery codes), and self-service password reset (`/api/v1/auth/password-reset/{request,confirm}` with 24h single-use tokens, anti-enumeration response, and session sweep on confirm) exist. Remaining work is user invite/deactivate flows, role management UI/API, and user-visible audit trails. |
